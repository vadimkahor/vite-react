...
export const updateGame = (
  ...
  // Обновление времени (timeScale=1 это примерно 1/60 секунды)
  ...
  if (refs.burnoutTimer > 0) {
      ...
      // Генерация густого дыма при пробуксовке
      if (Math.random() < 0.8 * timeScale) {
        ...
        for (let i = 0; i < axles.length; i++) {
            ...
            spawnParticle(
                refs,
                (isLeft ? p.x : p.x + p.width) + (Math.random() * 14 - 7),
                p.y + yOffset,
                (Math.random() - 0.5) * 6,
                (Math.random() * 2) + 2,
                5 + Math.random() * 6,
        ...
        if (Math.random() > 0.5) {
             ...
             refs.skidMarks.push({
                id: Math.random().toString(),
                x: Math.random() > 0.5 ? p.x + tireOffset : p.x + p.width - tireOffset - tireWidth,
                y: p.y + p.height - 5,
                width: tireWidth,
                height: 5 * timeScale,
                opacity: 0.5
  ...
  if (isAccelerating) {
    ...
    if (refs.gameState.playerSpeed > 2) {
        ...
        refs.skidMarks.push({
            id: Math.random().toString(),
            x: p.x + p.width - tireOffset - tireWidth,
            y: p.y + p.height - 15,
            width: tireWidth,
            height: markHeight,
            opacity: opacity
    ...
    if (refs.brakingDuration > 15) {
        const spawnChance = 0.6 * timeScale;
        const axles = [15, p.height - 5]; 
        ...
        for (let i = 0; i < axles.length; i++) {
            if (Math.random() < spawnChance) {
                ...
                spawnParticle(
                    refs,
                    (isLeft ? p.x : p.x + p.width) + (Math.random() * 10 - 5),
                    p.y + axles[i],
                    (Math.random() - 0.5) * 3,
                    (Math.random() * 2),
                    3 + Math.random() * 4,
  ...
  // --- ОПТИМИЗИРОВАННОЕ ОБНОВЛЕНИЕ МАССИВОВ ---
  ...
  for (let i = refs.skidMarks.length - 1; i >= 0; i--) {
      ...
      if (m.y >= canvas.height + 100 || m.opacity <= 0) {
          refs.skidMarks.splice(i, 1);
      }
  }

  // Обновление particles (Итерируемся по пулу)
  // Не используем splice, просто меняем флаг active
  for (let i = 0; i < refs.particles.length; i++) {
  ...
  if (currentSpeed > 9 && Math.random() < 0.25 * timeScale) {
    ...
    const xOffset = isLeft 
      ? -20 - (Math.random() * 40) 
      : LEVEL1_CONFIG.ROAD_WIDTH + 20 + (Math.random() * 40);
    
    refs.speedLines.push({
      id: Math.random().toString(),
      x: roadX + xOffset,
      y: -150,
      length: 80 + Math.random() * 150,
      speed: currentSpeed * (1.8 + Math.random() * 0.5),
  ...
  for (let i = refs.speedLines.length - 1; i >= 0; i--) {
      ...
      if (line.y >= canvas.height) {
          refs.speedLines.splice(i, 1);
      }
  }

  // 5. Дистанция
  // ОБНОВЛЕНО: Рассчитываем, чтобы 5000 метров проходились за 60 секунд на обычной скорости
  ...
  const isTurningLeft = keys.has('ArrowLeft');
  const isTurningRight = keys.has('ArrowRight');
  ...
  if (isTurningLeft || isTurningRight) {
      ...
      if (refs.turnDuration > 25 && currentSpeed > 8) {
          ...
          if (Math.random() < driftChance) {
             ...
             spawnParticle(
                refs,
                (isLeftTire ? p.x : p.x + p.width) + (Math.random() * 10 - 5),
                p.y + 15,
                (Math.random() - 0.5) * 3,
                (Math.random() * 2),
                3 + Math.random() * 4,
  ...
  if (p.x <= 0 || p.x >= LEVEL1_CONFIG.ROAD_WIDTH - LEVEL1_CONFIG.PLAYER_WIDTH) {
    ...
    if (p.x < 0) {
      p.x = 0;
      refs.playerVelocityX *= -0.5;
    }
    if (p.x > LEVEL1_CONFIG.ROAD_WIDTH - LEVEL1_CONFIG.PLAYER_WIDTH) {
      p.x = LEVEL1_CONFIG.ROAD_WIDTH - LEVEL1_CONFIG.PLAYER_WIDTH;
      refs.playerVelocityX *= -0.5;
    }
  }
  ...
  if (isBraking) {
      ...
      if (currentSpeed <= LEVEL1_CONFIG.INITIAL_SPEED) {
           surgeOffset = BASE_OFFSET;
      } else if (currentSpeed <= SURGE_SPEED_END) {
           const t = (currentSpeed - LEVEL1_CONFIG.INITIAL_SPEED) / (SURGE_SPEED_END - LEVEL1_CONFIG.INITIAL_SPEED);
           surgeOffset = BASE_OFFSET + (t * (SURGE_PEAK_OFFSET - BASE_OFFSET));
      } else {
           const t = (currentSpeed - SURGE_SPEED_END) / (LEVEL1_CONFIG.MAX_SPEED - SURGE_SPEED_END);
           surgeOffset = SURGE_PEAK_OFFSET - (t * (SURGE_PEAK_OFFSET - HIGH_SPEED_OFFSET));
      }
  ...
  let targetSnowAmp = 4;
  if (currentSpeed <= LEVEL1_CONFIG.INITIAL_SPEED) {
      const t = Math.max(0, (currentSpeed - LEVEL1_CONFIG.MIN_SPEED) / (LEVEL1_CONFIG.INITIAL_SPEED - LEVEL1_CONFIG.MIN_SPEED));
      targetSnowAmp = 5 - (t * 1);
  } else {
      const t = Math.min(1, (currentSpeed - LEVEL1_CONFIG.INITIAL_SPEED) / (LEVEL1_CONFIG.MAX_SPEED - LEVEL1_CONFIG.INITIAL_SPEED));
      targetSnowAmp = 4 - (t * 2);
  }
  ...
  if (isBraking) {
      ...
      if (delta > 0) smoothingFactor = 0.12; 
          ...
          if (currentSpeed < SURGE_SPEED_END) smoothingFactor = 0.04; 
          else smoothingFactor = 0.008; 
      }
  }
  ...
  for (let i = refs.traffic.length - 1; i >= 0; i--) {
      ...
      if (t.y >= canvas.height + 500 || t.y <= -500) {
          refs.traffic.splice(i, 1);
          refs.nearMissCooldowns.delete(t.id);
      }
  }
  ...
  for (let i = refs.floatingMessages.length - 1; i >= 0; i--) {
      ...
      if (m.life <= 0) {
          refs.floatingMessages.splice(i, 1);
      }
  }
