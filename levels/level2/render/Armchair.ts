...
export const ARMCHAIR_CONFIG = {
  WIDTH: 90,
  HEIGHT: 50
};

export const drawArmchair = (ctx: CanvasRenderingContext2D, p: Platform) => {
    const key = `platform_armchair_${p.width}_${p.height}`;
    const topMargin = 25; 
    ...
    const sprite = getSprite(key, p.width, totalH, (c) => {
        ...
        // Одна подушка (кресло)
        drawRoundedRect(c, seatX, cushionY, seatW, seatHeight, 8); c.fill();
